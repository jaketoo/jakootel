...
async def _(event):
    user = await event.get_sender()
    if user.id == 5940413527:
        await event.reply(
            "حاضر سيدي وتاج راسي جناب السيد جاكو حفظه الله ☻️"
        )
...
async def _(event):
    ...
    mvhmedevent = await edit_or_reply(event, "**بوياي جار**")
...
async def _(event):
    mvhmedevent = await edit_or_reply(event, "**- جار الغاء حظر جميع المستخدمين**")
    succ = 0
    total = 0
    flag = False
...
async def rm_deletedacc(show):
    ...
    if con != "تنظيف":
        ...
        if flag != " -r":
            ...
            mvhmedadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not mvhmedadmin:
